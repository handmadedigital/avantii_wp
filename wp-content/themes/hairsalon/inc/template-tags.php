...
function thim_get_entry_meta_author() {
	...
	$html .= esc_html__( 'by ', 'hairsalon' ) . sprintf( '<a href="%1$s" rel="author">%2$s</a>', esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ), esc_html( get_the_author() ) ) . '';
	$html .= '</span>';
...
function thim_get_entry_meta_category() {
	if ( is_single() ) {
		...
		$html .= '<span>' . esc_html__( 'Categories: ', 'hairsalon' ) . '</span>';
...
function thim_get_entry_meta_tags() {
	$tags_list = get_the_tag_list( '', esc_html__( ', ', 'hairsalon' ) );
	if ( $tags_list ) {
		return sprintf( '<span class="tags-links">' . esc_html__( 'Tag: %1$s', 'hairsalon' ) . '</span>', $tags_list ); // WPCS: XSS OK.
	}
...
if ( ! function_exists( 'thim_social_share' ) ) {
	function thim_social_share() {
		...
		echo '<span class="blog-hide" title="' . esc_html__( 'Click to show', 'hairsalon' ) . '"><i class="fa fa-share"></i><br/>' . esc_html__( 'share item', 'hairsalon' ) . '</span>';
		echo '<ul class="thim-social-share row">';
		do_action( 'thim_before_social_list' );

		$socials = get_theme_mod( 'group_sharing', array( 'facebook', 'twitter', 'pinterest', 'google', 'fancy' ) );
	...
	switch ( $social_name ) {
		...
		case 'pinterest':
			echo '<li class="pinterest">
				<a data-pin-do="buttonBookmark"  href="' . esc_url( "//www.pinterest.com/pin/create/button/" ) . '"><img src="' . esc_url( "//assets.pinterest.com/images/pidgets/pinit_fg_en_rect_gray_20.png" ) . '" alt="' . esc_html__( 'Pinterest', 'hairsalon' ) . '"/></a>
...
if ( ! function_exists( 'thim_new_comment_fields' ) ) {
	function thim_new_comment_fields( $fields ) {
		...
		$fields = array(
			'author' => '<p class="comment-form-author">' . '<input placeholder="' . esc_attr__( 'Name', 'hairsalon' ) . ( $req ? ' *' : '' ) . '" id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" ' . $aria_req . ' /></p>',
			'email'  => '<p class="comment-form-email">' . '<input placeholder="' . esc_attr__( 'Email', 'hairsalon' ) . ( $req ? ' *' : '' ) . '" id="email" name="email" type="text" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" ' . $aria_req . ' /></p>',
			'url'    => '<p class="comment-form-url">' . '<input placeholder="' . esc_attr__( 'Website', 'hairsalon' ) . '" id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>',
		);
		...
		<div class="content-comment">
			<div class="author">
				...
				<span>
					...
					<?php edit_comment_link( esc_html__( 'Edit', 'hairsalon' ), '', '' ); ?>
				</span>
			</div>
			<?php if ( $comment->comment_approved == '0' ) : ?>
				<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'hairsalon' ) ?></em>
			<?php endif; ?>
			<div class="message">
				<?php comment_text() ?>
			</div>
...
function thim_excerpt_length() {
	return get_theme_mod( 'excerpt_archive_content', 50 );
}
...
if ( ! function_exists( 'thim_archive_title' ) ) :
	function thim_archive_title( $before = '', $after = '' ) {
		if ( is_category() ) {
			$title = sprintf( esc_html__( '%s', 'hairsalon' ), single_cat_title( '', false ) );
		} elseif ( is_tag() ) {
			$title = sprintf( esc_html__( '%s', 'hairsalon' ), single_tag_title( '', false ) );
		} elseif ( is_author() ) {
			$title = sprintf( esc_html__( '%s', 'hairsalon' ), '<span class="vcard">' . get_the_author() . '</span>' );
		} elseif ( is_year() ) {
			$title = sprintf( esc_html__( 'Year: %s', 'hairsalon' ), get_the_date( _x( 'Y', 'yearly archives date format', 'hairsalon' ) ) );
		} elseif ( is_month() ) {
			$title = sprintf( esc_html__( 'Month: %s', 'hairsalon' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'hairsalon' ) ) );
		} elseif ( is_day() ) {
			$title = sprintf( esc_html__( 'Day: %s', 'hairsalon' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'hairsalon' ) ) );
		} elseif ( is_tax( 'post_format', 'post-format-aside' ) ) {
			$title = _x( 'Asides', 'post format archive title', 'hairsalon' );
		} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
			$title = _x( 'Galleries', 'post format archive title', 'hairsalon' );
		} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
			$title = _x( 'Images', 'post format archive title', 'hairsalon' );
		} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
			$title = _x( 'Videos', 'post format archive title', 'hairsalon' );
		} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
			$title = _x( 'Quotes', 'post format archive title', 'hairsalon' );
		} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
			$title = _x( 'Links', 'post format archive title', 'hairsalon' );
		} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
			$title = _x( 'Statuses', 'post format archive title', 'hairsalon' );
		} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
			$title = _x( 'Audio', 'post format archive title', 'hairsalon' );
		} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
			$title = _x( 'Chats', 'post format archive title', 'hairsalon' );
		} elseif ( is_post_type_archive() ) {
			$title = sprintf( esc_html__( '%s', 'hairsalon' ), post_type_archive_title( '', false ) );
		} elseif ( is_tax() ) {
			$tax = get_taxonomy( get_queried_object()->taxonomy );
			/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
			$title = sprintf( esc_html__( '%1$s: %2$s', 'hairsalon' ), $tax->labels->singular_name, single_term_title( '', false ) );
		} elseif ( is_404() ) {
			$title = esc_html__( '404 Page', 'hairsalon' );
		} elseif ( is_search() ) {
			$title = esc_html__( 'Search Results Page', 'hairsalon' );
		} else {
			$title = esc_html__( 'Archives', 'hairsalon' );
		}
		/**
